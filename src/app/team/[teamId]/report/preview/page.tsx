    ...
    <div className="grid grid-cols-1 md:grid-cols-2 relative">
      ...
      <div className="p-6 md:p-12 flex flex-col justify-center">
        ...
        <div className="pt-3 md:pt-4 border-t" style={{ borderColor: 'rgba(255, 215, 0, 0.1)' }}>
          <p className="font-mono text-gray-600 mb-2" style={{ fontSize: '9px', letterSpacing: '2px' }}>TEAM ROSTER</p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 relative">
      ...
      <div className="p-5 md:p-7 relative md:border-r" style={{ borderColor: 'rgba(255, 215, 0, 0.15)' }}>
        ...
        {polishedCard.narrative && (
          <div>
            <p className="font-mono font-bold tracking-widest mb-2" style={{ fontSize: '8px', color: 'rgba(255, 255, 255, 0.5)', letterSpacing: '2px' }}>
              ◆ NARRATIVE
            </p>
            <div className="leading-relaxed whitespace-pre-wrap" style={{ fontSize: '12.5px', color: 'rgba(255, 255, 255, 0.85)', lineHeight: 1.75 }}>
              {polishedCard.narrative}
            </div>
          </div>
        )}
      ...
      <div className="p-5 md:p-7 relative">
        ...
        {card.oneSentenceStrategy && (
          <div className="rounded-xl p-4 mb-4 relative overflow-hidden"
            ...
            <p className="text-white leading-relaxed font-medium" style={{ fontSize: '13px' }}>{card.oneSentenceStrategy}</p>
          </div>
        )}
        {polishedCard.bridge && (
          <div className="rounded-lg p-3" style={{ background: 'rgba(255, 255, 255, 0.03)', border: `0.5px dashed ${cardColor}50` }}>
            ...
            <p className="leading-relaxed italic" style={{ fontSize: '11.5px', color: 'rgba(255, 255, 255, 0.7)' }}>{polishedCard.bridge}</p>
          </div>
        )}
        <div className="mt-4 md:mt-0 md:absolute md:bottom-3 md:right-7 font-mono text-center md:text-left"
          style={{ fontSize: '9px', color: 'rgba(255,255,255,0.2)', letterSpacing: '2px' }}>
          PAGE {String(pageIndex + 1).padStart(2, '0')}<span className="hidden md:inline"> · RIGHT</span>
        </div>
      </div>
    </div>
  );
...
function CardSignature({ cardId, color }: { cardId: string; color: string }) {
  ...
  return (
    <div className="rounded-lg flex items-center justify-center font-black flex-shrink-0"
      style={{ width: '38px', height: '38px', background: color, color: isLight ? '#111' : '#fff', fontSize: '13px', fontFamily: 'monospace' }}>
      {cardId}
    </div>
  );
}
...
function DifficultyStars({ level, color }: { level: number; color: string }) {
  return (
    <div className="flex items-center gap-0.5 flex-shrink-0">
      {[1, 2, 3, 4, 5].map(i => (
        <span key={i} style={{ fontSize: '10px', color: i <= level ? color : 'rgba(255,255,255,0.15)' }}>★</span>
      ))}
    </div>
  );
}
