...
const LEVELS: Record<string, { label: string; emoji: string; timer: number; minChars: number; color: string; desc: string }> = {
  basic: { label: '초급', emoji: '🌱', timer: 1800, minChars: 20, color: '#059669', desc: '처음 시작 팀 · 30분 · 최소 20자' },
  standard: { label: '표준', emoji: '📘', timer: 1200, minChars: 50, color: '#00B4D8', desc: '일반 수업 · 20분 · 최소 50자' },
  advanced: { label: '심화', emoji: '🚀', timer: 900, minChars: 100, color: '#7F77DD', desc: '심화반 · 15분 · 최소 100자' },
};
...
export default function Home() {
  ...
  // Check if a sub-card is unlocked
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (showActivity || screen !== 'game') return;
  ...
  }, [currentIndex, goTo, showActivity, screen]);
  ...
  if (screen === 'landing') return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-md">
        ...
        <button onClick={() => setScreen('guide')} className="w-full py-3 bg-gray-800 text-gray-400 rounded-2xl text-sm border border-gray-700 transition hover:bg-gray-700">📋 퍼실리테이터 가이드 (교수자용)</button>
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center px-4 py-6 overflow-auto">
        <div className="w-full max-w-md">
            ...
            <div className="grid grid-cols-2 gap-2">
              {ITEMS.map(it => (
                <button key={it} onClick={() => setItem(it)}
                  className={`px-3 py-3 rounded-xl text-left text-[12px] border transition ${item === it ? 'bg-[#0B1E3D] text-white border-cyan-500 font-bold' : 'bg-gray-900 text-gray-300 border-gray-700'}`}>
                  {it}
                </button>
              ))}
            </div>
          ...
          {/* 2. 역할 + 이름 */}
          <div className="mb-6">
            <p className="text-sm font-bold text-white mb-1">② 역할 · 이름 입력</p>
            <div className="flex gap-2 mb-3">
              {([['leader', '👑 팀장', '결론 작성 · 완료 처리'], ['member', '💬 팀원', '토론 참여 · 카드 열람']] as const).map(([k, l, d]) => (
          ...
          {/* 3. 수준 선택 */}
          <div className="mb-6">
            <p className="text-sm font-bold text-white mb-1">③ 수업 수준 선택</p>
            <p className="text-[11px] text-gray-500 mb-3">수준에 따라 제한시간·최소 작성량이 조정됩니다</p>
            {Object.entries(LEVELS).map(([k, v]) => (
              <button key={k} onClick={() => setLevel(k)}
                ...
                <span className={`text-[11px] ${level === k ? 'text-white/80' : 'text-gray-500'}`}>
                  {Math.floor(v.timer / 60)}분 · 최소 {v.minChars}자
                </span>
              </button>
            ))}
          </div>
          ...
          <button onClick={startGame} disabled={!canStart}
            className="w-full py-4 bg-cyan-500 text-white font-bold rounded-2xl text-base transition hover:bg-cyan-600 disabled:opacity-40 disabled:cursor-not-allowed mb-3">
            시작하기 →
          </button>
          <button onClick={() => setScreen('guide')} className="w-full py-3 text-gray-500 text-[11px] border border-gray-800 rounded-xl transition hover:bg-gray-900">
            📋 퍼실리테이터 가이드 보기 (교수자용)
          </button>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-[#0B1E3D] to-[#1A3A6B] rounded-2xl p-6 mb-6">
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            <span className="text-[10px] bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full font-bold">🕐 50~80분</span>
            <span className="text-[10px] bg-cyan-500/20 text-cyan-400 px-3 py-1 rounded-full font-bold">👥 팀별 4~6명</span>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          {[
            { time: '0–5분', step: '카드 개념 확인', icon: '🎯', color: '#6366F1', action: '주제카드 앞면을 읽고 뒷면의 개념을 확인합니다.', tip: '카드를 프로젝터에 띄워 전체가 함께 확인하세요.' },
            { time: '5–20분', step: '팀 토론', icon: '💬', color: '#D97706', action: '핵심 통찰 질문에 대해 팀원 모두가 의견을 나눕니다.', tip: '"외국 친구에게 설명한다면?" 으로 유도하세요.' },
            { time: '20–40분', step: '체크리스트 + 팀장 결론', icon: '✏️', color: '#059669', action: '체크리스트 확인 후 팀장이 최종 결론을 작성합니다.', tip: 'AI 피드백은 체크리스트 + 최소 글자수 충족 시 활성화됩니다.' },
            { time: '40–50분', step: 'AI 피드백 → 완료', icon: '🤖', color: '#0E7490', action: '팀장이 AI 피드백을 받아 보완 후 완료 버튼 클릭.', tip: '시간 부족 팀은 AI 자동 작성을 허용하되 수정·컨펌하도록 지도.' },
            { time: '50–60분', step: '팀별 발표 및 정리', icon: '📌', color: '#DC2626', action: '1~2팀이 팀장 결론을 발표합니다.', tip: '결과물은 화면 캡처 또는 PDF 저장 가능합니다.' },
          ].map((f, i) => (
            <div key={i} className="flex gap-3 mb-4">
              <div className="flex flex-col items-center">
                ...
                {i < 4 && <div className="w-0.5 h-4 bg-gray-800 mt-1" />}
              </div>
              <div className="flex-1 pt-1">
                ...
                <div className="text-[12px] text-gray-300 bg-gray-900 rounded-lg p-3 mb-1 leading-relaxed">{f.action}</div>
                <div className="text-[11px] rounded-lg px-3 py-1.5" style={{ color: f.color, background: f.color + '10' }}>💡 {f.tip}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* 역할별 권한 */}
        <div className="mb-8">
          <h3 className="text-base font-bold text-white mb-4">역할별 권한</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4">
              <div className="text-[14px] font-bold text-amber-400 mb-2">👑 팀장</div>
              {['답변 입력 · 결론 작성', '완료 처리 → 다음 카드 해제', 'AI 피드백 요청', 'AI 자동 초안 요청·수정·컨펌'].map(t => (
                <div key={t} className="flex items-start gap-2 mb-1"><span className="text-green-400 text-[11px] mt-0.5">✅</span><span className="text-[12px] text-gray-300">{t}</span></div>
              ))}
            </div>
            <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl p-4">
              <div className="text-[14px] font-bold text-cyan-400 mb-2">💬 팀원</div>
              {['카드 앞뒷면 열람', '체크리스트 자가 점검', '팀장 답변 실시간 열람'].map(t => (
                <div key={t} className="flex items-start gap-2 mb-1"><span className="text-green-400 text-[11px] mt-0.5">✅</span><span className="text-[12px] text-gray-300">{t}</span></div>
              ))}
              <div className="flex items-start gap-2 mb-1"><span className="text-red-400 text-[11px] mt-0.5">✕</span><span className="text-[12px] text-gray-500">결론 작성 / AI 요청 / 완료 처리</span></div>
            </div>
          </div>
        </div>
        ...
        {/* 오답 패턴 */}
        <div className="mb-8">
          <h3 className="text-base font-bold text-white mb-4">자주 나오는 오답 패턴</h3>
          {[
            { err: '산업을 너무 좁게 정의', fix: "'K-뷰티' → '화장품·퍼스널케어 산업'으로 넓히도록 유도" },
            { err: '고객을 "모든 사람"으로 설정', fix: '"가장 먼저 살 것 같은 사람 한 명을 먼저 생각해보세요"' },
            { err: 'AI 답변만 복붙', fix: '최소 글자수 조건으로 시스템이 방지. 체크리스트 직접 체크 지도' },
            { err: '팀장만 글 쓰고 팀원은 방관', fix: '팀원 전원 의견 입력 후 팀장 결론으로 이동하도록 순서 안내' },
          ].map((p, i) => (
            <div key={i} className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 mb-2">
              <div className="text-[12px] font-bold text-orange-400 mb-1">⚠ {p.err}</div>
              <div className="text-[12px] text-orange-200/80">→ {p.fix}</div>
            </div>
          ))}
        </div>
        ...
        {/* 수준별 팁 */}
        <div className="mb-8">
          <h3 className="text-base font-bold text-white mb-4">수준별 핵심 팁</h3>
          {Object.entries(LEVELS).map(([k, v]) => (
            <div key={k} className="rounded-xl p-3 mb-2" style={{ background: v.color + '15', border: `1px solid ${v.color}33` }}>
              <div className="text-[12px] font-bold mb-1" style={{ color: v.color }}>{v.emoji} {v.label} — {Math.floor(v.timer/60)}분 · 최소 {v.minChars}자</div>
              <div className="text-[12px] text-gray-300">
                {k === 'basic' && '질문을 쉬운 말로 풀어 읽어주세요. AI 자동 작성 적극 허용하되 수정 필수.'}
                {k === 'standard' && '팀 토론 2분 후 팀장 결론 탭 이동 유도. AI 피드백은 결론 작성 후 활성화.'}
                {k === 'advanced' && 'HS코드 조사를 사전 과제로 부여. AI 자동 작성은 최후 수단으로 제한.'}
              </div>
            </div>
          ))}
        </div>

        <button onClick={() => setScreen('landing')} className="w-full py-3 bg-[#0B1E3D] text-white font-bold rounded-xl text-sm mb-8">← 학생 화면으로 돌아가기</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-[#1a1a2e] to-[#16213e] flex flex-col items-center px-4 py-5 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowList(!showList)}
              className="bg-white/10 border border-white/15 rounded-lg px-2.5 py-1 text-[11px] text-gray-300 hover:bg-white/15 transition">
      ...
      {timer <= 30 && timer > 0 && isLeader && (
        <div className="w-full max-w-md mb-3 relative z-10 bg-orange-500/15 border border-orange-500/30 rounded-xl px-4 py-3">
          ...
          <div className="text-[11px] text-orange-300/80">미완료 카드에 빠르게 답변을 작성하세요.</div>
        </div>
      )}
          ...
          {TOPICS.map((topic) => (
            <div key={topic.id} className="mb-4">
              ...
              <div className="flex gap-1.5 pl-10">
                {topic.subs.map((sub) => {
                  const locked = !isUnlocked(sub.id);
                  const done = completedCards.has(sub.id);
                  return (
                    <button key={sub.id} onClick={() => { if (!locked) { goTo(ALL_CARDS.findIndex(c => c.data.id === sub.id)); setShowList(false); } }}
                      ...
                      style={{
                        background: done ? `${CARD_COLORS[topic.id].bg}22` : 'rgba(255,255,255,0.04)',
                        border: `1px solid ${done ? CARD_COLORS[topic.id].bg + '55' : 'rgba(255,255,255,0.08)'}`,
                        color: done ? CARD_COLORS[topic.id].bg : locked ? '#666' : '#aaa',
                      }}>
                      {locked ? '🔒' : done ? '✓' : ''} {sub.id}
                    </button>
                  );
      ...
      <div className="w-full max-w-[340px] relative z-10 cursor-pointer"
        ...
        onClick={() => !showActivity && !isCardLocked && setIsFlipped(f => !f)}
        ...
        {isCardCompleted && !isCardLocked && (
          <div className="absolute top-3 right-3 z-20 bg-green-500 text-white text-[10px] font-bold px-3 py-1 rounded-full shadow-lg">✓ 완료</div>
        )}
      ...
      {/* Hint text */}
      <p className="text-[11px] text-gray-600 mt-2.5 text-center relative z-10">
        {isCardLocked ? '🔒 이전 카드를 완료하세요' :
         isFlipped ? (card.type === 'question' && isLeader ? '전략 작성하기 버튼 클릭' : '뒷면 보는 중') : '탭하여 뒤집기'} · 스와이프로 이동
      </p>
      ...
      {/* Sub-card quick nav with lock/complete status */}
          ...
          {TOPICS[currentTopicIdx].subs.map((sub, i) => {
            ...
            return (
              <button key={sub.id} onClick={() => !locked && goTo(ALL_CARDS.findIndex(c => c.data.id === sub.id))}
                ...
                style={{
                  background: card.data.id === sub.id ? color : done ? color + '33' : 'rgba(255,255,255,0.06)',
                  border: `1px solid ${card.data.id === sub.id ? color : 'rgba(255,255,255,0.1)'}`,
                  color: done ? color : locked ? '#666' : '#fff',
      ...
      {isLeader && card.type === 'question' && !isCardLocked && !isCardCompleted && hasResponse(card.data.id) && (
        <button onClick={() => completeCard(card.data.id)}
          className="mt-3 px-6 py-2.5 bg-green-500 text-white font-bold rounded-xl text-[13px] relative z-10 shadow-lg shadow-green-500/25 transition hover:bg-green-600">
          ✅ 이 카드 완료하기
        </button>
      )}
      ...
      {!isLeader && card.type === 'question' && !isCardLocked && (
        <div className="mt-3 px-4 py-2 bg-cyan-500/10 border border-cyan-500/20 rounded-xl text-[11px] text-cyan-400 relative z-10 text-center">
          💬 팀원 모드 · 체크리스트 확인 가능 · 결론 작성은 팀장만 가능
        </div>
      )}
