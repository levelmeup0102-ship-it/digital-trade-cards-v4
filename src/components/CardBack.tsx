    ...
    <div className="w-full h-full flex flex-col p-5 bg-white rounded-2xl border border-gray-200 shadow-lg overflow-auto" onClick={(e) => e.stopPropagation()}>
      ...
      {isTopic ? (
        <>
          ...
          <p className="text-sm font-extrabold leading-snug text-gray-900">{(data as TopicCard).insightQ}</p>
        </>
      ) : (
        <>
          ...
          <div className="flex flex-col gap-2 mb-4">
            {(data as SubCard).checklist.map((item, i) => (
              <label key={i} onClick={(e) => e.stopPropagation()} className="flex items-start gap-2 cursor-pointer text-[12.5px] leading-relaxed text-gray-700">
                <input type="checkbox" checked={checkStates[i] || false} onChange={(e) => { e.stopPropagation(); onCheck(i); }} className="mt-0.5 w-4 h-4 flex-shrink-0" style={{ accentColor: color }} />
                <span style={{ textDecoration: checkStates[i] ? 'line-through' : 'none', opacity: checkStates[i] ? 0.5 : 1 }}>{item}</span>
              </label>
            ))}
          </div>
          {onOpenActivity && (
            <button onClick={(e) => { e.stopPropagation(); onOpenActivity(); }} className="w-full py-3 rounded-xl text-[13px] font-bold flex items-center justify-center gap-1.5 transition" style={{ background: hasResponse ? color + '15' : color, border: hasResponse ? '1.5px solid ' + color : 'none', color: hasResponse ? color : '#fff' }}>
              {hasResponse ? '✓ 작성 완료 · 수정하기' : '✏ 전략 작성하기'}
            </button>
          )}
        </>
      )}
